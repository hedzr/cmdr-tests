...
func AttachServerCommand(parent cli.CommandBuilder) { //nolint:revive
	...
	b := parent.Cmd("start", "s", "st", "run", "startup").
	...
	b = parent.Cmd("run1", "nf", "startup1").
		Description("startup this system service/daemon", ``).
		OnAction(serverStartup)
	b.Cmd("run1", "nf", "startup1").
		Description("startup this system service/daemon", ``).
		OnAction(serverStartup).
		Build()
	b.Build()
...
func tagsCommands(parent cli.CommandBuilder) { //nolint:revive
	...
	b1 := b.Cmd("check-point", "pt", "chk-pt").
	...
	b = parent.Cmd("rm", "r", "remove", "delete", "del", "erase").
	...
	b = parent.Cmd("modify", "m", "mod", "modi", "update", "change").
	...
	b = parent.Cmd("toggle", "t", "tog", "switch").
...
func tgCommand(parent cli.CommandBuilder) { //nolint:revive
	...
	b := parent.Cmd("tg-test", "tg", "toggle-group-test").
	...
	b = parent.Cmd("tg-test2", "tg2", "toggle-group-test2").
...
func cmdrManyCommandsTest(parent cli.CommandBuilder) {
	for i := 1; i <= 23; i++ {
		...
		b := parent.Cmd(t, fmt.Sprintf("sc%v", i)).
